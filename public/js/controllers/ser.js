    ...
    function saveSMS() {
        ...
        function prepare(params, isPost){
            var obj = {
                ...
                notCallback : true,
                goformId : "SAVE_SMS",
                SMSMessage : escapeMessage(encodeMessage(params.message)), //????
                SMSNumber : params.numbers.join(";") + ";",//?????
                Index : params.index,//-1?????????????????????????ID
                encode_type : getEncodeType(params.message).encodeType,
                sms_time : params.currentTimeString,
                draft_group_id : params.groupId //?????
            };
            return obj;
        }
